...
- **Un-ignore restores a file** — removing the `n8n:ignore` tag from a managed file unarchives its workflow in n8n and returns the file to its mapping's default mode (`sync` or `link`), the inverse of hand-tagging it ignored. Driven by a tag-removal listener (`TagUnassignedEvent`).
- **Moving a duplicate workflow file into a mapping that already syncs it merges** — when an unmapped copy carrying an existing workflow's `n8n_id` lands in a folder where that workflow is already synced, the incoming copy is deleted (the existing synced file is the source of truth) and nothing is restored or duplicated in n8n.
...
- Unit (`MotionService::moveIn` sibling-by-id scan) + live `move.feature` coverage for **merge-on-collision** — moving an unmapped copy into a mapping that already syncs the same workflow deletes the incoming copy (under the SyncGuard, so n8n is untouched) and keeps the existing synced file; a sibling carrying a *different* id still takes the normal unarchive path. This closes the last `move.feature` `@todo`.
