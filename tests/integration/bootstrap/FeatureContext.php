...
final class FeatureContext implements Context {
	...
	// ── move / motion steps (saga §14.2: move-out → unmapped+archive, move-in → restore) ──
	// MoveGuardListener vets the move on BeforeNodeRenamedEvent (sync may leave,
	// link may not); MotionListener applies the n8n-side consequence on the
	...
	// ── copy lifecycle (copy.feature) ─────────────────────────────────────────
	// A COPY is the opposite of a MOVE: always a brand-new instance, never the
	// original's identity. NC fires NodeCopiedEvent, which CopyListener handles by
	// stripping any inherited metadata and (if the copy landed in a mapping)
	// registering it as a fresh workflow.
