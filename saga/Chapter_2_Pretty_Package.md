...
> a CI pipeline can ship, and eventually the Nextcloud app store can accept (Chapter 4).
...
4. → *transition* → the integration suite is the safety net that makes the **second refactor**
   (the mode-model overhaul + motion) safe to attempt at all — and that refactor grew big enough
   to open its own act, **Chapter 3 — An Audition**.
5. → *transition: **branding*** (Ch4 §4.1) → once the app is market-ready, the work turns to identity.
...
| 6 | **Second refactor + edge-case features** — the mode-model overhaul (sync/link/unmapped; drop backup/writeback) **+** the motion lifecycle (move-out/restore, copy-strips, merge), *made safe by epic 5* | → Ch3 | An Audition |
| 7 | **Secondary refactor** — a cleanup pass once the edge-case work reveals the real shape (scope TBD) | → Ch3 | An Audition (follow-on) |

Epics 1–5 are delivered — **that's this chapter**: it closes at epic 5, the marker where the
**integration suite runs green in the pipeline**. Epics 6–7 (the mode/motion refactor the
integration suite was built to make safe) open the next act, **Chapter 3 — An Audition**.
**Branding** (Ch4 §4.1) comes later still, once Kelly judges the app market-ready.
...
> **A new crew member joins.** Every ragtag crew picks up another member somewhere along the
> road, and this is where ours does: enter **Behat** — a mighty warrior who fights in plain
> speech, his shield bearing a **pickle insignia** (the Gherkin he speaks is, after all, just a
> small green pickle). The unit-test scouts had already mapped the terrain in mocks; Behat is the
> one who actually *walks it* — striding onto a live Nextcloud + a live n8n and swinging at the
> real thing, scenario by scenario, until both sides concede. He doesn't argue in JSON or `occ`
> dialects; he states what *should be true* in the common tongue, then proves it. From here on he
> marches at the front of the party — the safety net made flesh — and every refactor the heroes
> dare from this point forward is one Behat has agreed to guard. The crew is bigger now, and
> braver for it.
...
### 14. The payoff moved to its own act — **Chapter 3 — An Audition**

The mode-model & motion refactor that all this testing + devops was built *for* grew big enough to
deserve its own chapter. It now lives in
**[Chapter 3 — An Audition](./Chapter_3_An_Audition.md)**: the second round of coding that the
safety net made safe to attempt — the four-value `mode` collapse, the move/copy/restore lifecycle,
the manual reconcile, the sync⇄link mode-change, and the double-feature cadence (two agents, one
PR). Chapter 2 ends right here, at the marker that opens Chapter 3: **the integration suite running
green in the pipeline** — the hinge where the story turns from *"we have all the devops and
workflows"* to *"now we can actually code."*
