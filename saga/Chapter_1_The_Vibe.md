...
- **Storage-type gotcha (important, corrected in §17.3.3):** declarative INTERNAL storage records the **checkbox as bool-typed** but `TEXT`/string fields as **string-typed**. So `getValueBool('schedule_enabled')` works (same as `PushService` reads `api_enabled`) but `getValueInt('schedule_interval')` **throws `AppConfigTypeConflictException`**. `ScheduledPullJob` reads **defensively** (typed getter → fall back to a string parse) so it's robust either way. ⚠️ The premise here turned out to be half-wrong — see §17.3.3: a checkbox only round-trips as a bool if its schema `default` is a **real bool**, and `schedule_enabled` shipped with a string `'0'` default, which silently broke the toggle.
...
### 17.3.3 The checkbox that wouldn't stick — `schedule_enabled` never persisted (FIXED)
Smoke-testing the tag-sync **pull** leg (add a tag in n8n → expect it in NC) stalled because the scheduled pull never fired. `background-job:list` showed `ScheduledPullJob` last-run pinned at `1970-01-01T00:00:00` — it had **literally never run**. Working backwards:
- `config:list n8n_sync` → `schedule_enabled` was **absent entirely**, while sibling fields on the *same* form persisted (`schedule_interval: "5m"`, a TEXT field, saved fine). So the job was correctly no-opping on a value that was never there.
- **The premise in §17.3.1 was wrong.** Read the actual NC33 source on the pod: `lib/private/Settings/DeclarativeManager.php` `saveInternalValue()` writes **every** admin field via `setValueString()` (one code path, no per-type branch), and `getInternalValue()` returns the stored value (or the schema `default`) **with zero coercion**. There is no "checkbox is bool-typed" storage magic.
- **What actually makes a checkbox work:** the frontend round-trips a checkbox as a real boolean, so its schema `default` **must be a bool**. NC core proves it — `apps/dav/lib/Settings/Admin/SystemAddressBookSettings.php` declares `'default' => false`. Our `AutoSyncSettings` shipped `schedule_enabled` with `'default' => '0'` (a **string**). That string default broke the boolean round-trip, so ticking the box in the UI silently persisted nothing.
- **The backend was never the problem.** Setting `config:app:set n8n_sync schedule_enabled --value=1` by hand and `background-job:execute --force-execute` ran the pull in **16 ms** — `isEnabled()`'s defensive typed-getter-then-string-parse read handled the string fine. The gate was purely the UI never writing the value.
- **Fix (one line):** `schedule_enabled` default `'0'` → `false` in `AutoSyncSettings`. Redeployed via `deploy-dev.sh`; confirmed on the pod (`AutoSyncSettings.php` line shows `'default' => false`), `schedule_enabled=1`, `interval=5m`, cron alive (last tick ~30 s prior). The scheduled n8n→NC pull — and with it the pull-side tag reconcile — is unblocked.

**Lesson for the next agent:** declarative INTERNAL storage is **string-in / string-out with no coercion**; make a checkbox `default` a real `bool` and a text/number field a string. The old "bool-typed checkbox" note was a red herring — a checkbox appears to read back as a bool *only because the frontend and a bool default agree*, not because storage typed it.
